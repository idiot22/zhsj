...
Page({
  ...
  formSubmit: function (e) {
    ...
    if (that.data.schoolId == -1) {
      ...
      userPwd = sha.hex_sha1(md.hex_md5(userPwd)); //密码加密
      let schoolId=that.data.multiArray[2][1].id
      wx.request({
        method: 'POST',
        url: rootPath + "/login/stemLogin",
        header: {
        ...
        success(res) {
          ...
          if (res.data.code == 202) {
            ...
            for (let i in res.header){
              if (i == 'Set-Cookie'){
                getApp().globalData.header.Cookie = res.header[i];
                console.log(getApp().globalData.header.Cookie)
              }
            }
            console.log(res.header)
            // getApp().globalData.header.schoolId = that.data.multiArray[2][1].id;
            // getApp().globalData.header.studentId = res.cookies[1].substring(23, res.cookies[1].length - 47);
            // getApp().globalData.header.gradeId = res.cookies[3].substring(21, res.cookies[3].length - 47);
            wx.switchTab({
              url: '../index',
            })
