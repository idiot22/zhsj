...
baseComponent({
    ...
    methods: {
        ...
        onConfirm(e) { 
            ...
            const { index, item } = e.currentTarget.dataset  //idiot修改
            let shuju = this.data.options[index].children

            let pages = getCurrentPages()
            let currpage,prepage
            if(pages.length>=2){
              currpage = pages[pages.length-1]
              prepage = pages[pages.length-1]
            }
            if(prepage){
              prepage.filterCourse(shuju,flag,this.data.first)
            }
        },
        /**
